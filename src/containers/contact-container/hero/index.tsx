    ...
    >
      <div className="container-auto relative top-[98px] z-20 flex flex-col items-center">
        <Reveal className="flex justify-center">
          <div className="text-center space-y-4 max-w-[778px] text-white">
            <p className="text-xs font-semibold">
              SİZE NASIL YARDIMCI OLABİLİRİZ? BİZE ULAŞIN!
            </p>
            <h1 className="text-xl md:text-2xl leading-[48px] font-semibold">
              Herhangi bir sorunuz mu var?
            </h1>
            <p className="leading-6 md:leading-7 text-xs md:text-sm">
        ...
        <div className="bg-white min-h-[688px] w-full mt-24 py-[72px] px-10 shadow-reference-card rounded-[40px] grid grid-cols-1 md:grid-cols-2 gap-8">
          <Reveal axis="x" className="flex items-center h-full">
              ...
              <div className="space-x-4 flex">
                {socialLinks.map((link, _idx) => (
                  <Link
                    href={link.href}
                    key={_idx}
                    className="w-12 h-12 bg-cream rounded-2xl text-primary grid place-items-center hover:bg-blue hover:text-white"
                  >
                    {link.icon}
                  </Link>
                ))}
